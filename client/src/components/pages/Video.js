...
export default function Video(props) {
  ...
  return (
    <Grid container style={{ height: "100%", minHeight: "85vh" }}>
      ...
      <div style={{height: "100%",width: "100%",position: "relative"}}>
        ...
        >{console.log(tracks)}
        <p style={{ position: "absolute",color: "red",zIndex:"1" }}>You {(trackState.video) ? <VideocamIcon /> : <VideocamOffIcon />} {trackState.audio ? <MoreHorizIcon /> : <MicOffIcon />} </p>
        </AgoraVideoPlayer>
      </div>
      </Grid>
      {lusers.length > 0 &&
        lusers.map((user) => {
          ...
          if (user.videoTrack) {
            return (
              <Grid key={user.uid} item xs={gridSpacing} style={{backgroundColor:"dimgrey"}}>
                <div style={{height: "100%",width: "100%",position: "relative"}}>
                    <AgoraVideoPlayer
            ...
            return (
              <Grid key={user.uid} item xs={gridSpacing} style={{backgroundColor:"dimgrey"}}>
                <div style={{height: "100%",width: "100%",position: "relative"}}>
                <p style={{width: "100%",position: "absolute",color: "lightgrey",zIndex:"1"}}>{user.uid}{(user.videoTrack) ? <VideocamIcon /> : <VideocamOffIcon />} {user.audioTrack ? <MoreHorizIcon /> : <MicOffIcon />}</p>
                </div>
